            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="space-y-5">
                            ...
                            <div>
                                ...
                                <div id="logo-preview-container"
                                    class="mb-5 p-5 rounded-xl border border-surface-700 {{ !empty($settings['company_logo']) ? 'inline-block' : 'hidden' }} min-w-[200px] checkerboard">
                                    <img id="logo-preview-image"
                                        src="{{ !empty($settings['company_logo']) ? Storage::url($settings['company_logo']) : '#' }}"
                                        alt="Current Logo"
                                        class="max-h-24 w-auto object-contain mx-auto mix-blend-normal" />
                                </div>
                                ...
                                <p class="mt-2 text-xs text-surface-500">PNG, JPG or GIF up to 2MB. Recommended height:
                                    200px. Selected logo should be in horizontal format.</p>
                                <p id="logo-size-error" class="mt-2 text-xs text-danger-400 hidden">File size exceeds 2MB
                                    limit!</p>
                                @error('company_logo')
                    ...
                    <div class="glass rounded-2xl p-6">
                        <div class="flex items-center justify-between mb-5">
                            <h3 class="text-lg font-semibold text-white">Email Theme</h3>
                            <label class="relative inline-flex items-center cursor-pointer group">
                                <input type="checkbox" name="email_theme_enabled" class="sr-only peer"
                                    {{ ($settings['email_theme_enabled'] ?? '0') == '1' ? 'checked' : '' }}>
                                <div
                                    class="w-11 h-6 rounded-full transition-all duration-300 bg-surface-700 peer-checked:bg-primary-500 relative
                                after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white 
                                after:rounded-full after:h-5 after:w-5 after:transition-all after:duration-300
                                peer-checked:after:translate-x-full peer-checked:after:scale-90 peer-active:after:w-6
                                shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)]">
                                </div>
                                <span
                                    class="ml-3 text-sm font-medium text-surface-400 group-hover:text-surface-200 transition-colors">Enable
                                    Theme</span>
                            </label>
                        </div>
                        <p class="text-sm text-surface-400 mb-5">Select the master layout that will wrap all your outgoing
                            emails. If disabled, a plain email will be sent.</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <label class="relative cursor-pointer">
                                <input type="radio" name="email_theme" value="default" class="peer sr-only"
                                    {{ $currentTheme === 'default' ? 'checked' : '' }}>
                                <div
                                    class="p-4 rounded-xl border-2 border-surface-700 bg-surface-800 peer-checked:border-primary-500 peer-checked:bg-primary-500/5 transition-all text-center">
                                    <div
                                        class="w-full h-24 bg-white rounded-md mb-3 flex flex-col pt-3 px-2 border border-surface-600">
                            ...
                            <!-- Dark Theme -->
                            <label class="relative cursor-pointer">
                                <input type="radio" name="email_theme" value="dark" class="peer sr-only"
                                    {{ $currentTheme === 'dark' ? 'checked' : '' }}>
                                <div
                                    class="p-4 rounded-xl border-2 border-surface-700 bg-surface-800 peer-checked:border-primary-500 peer-checked:bg-primary-500/5 transition-all text-center">
                                    <div
                                        class="w-full h-24 bg-surface-900 rounded-md mb-3 flex flex-col pt-3 px-2 border border-surface-600 shadow-inner">
                            ...
                            <!-- Bold Theme -->
                            <label class="relative cursor-pointer">
                                <input type="radio" name="email_theme" value="bold" class="peer sr-only"
                                    {{ $currentTheme === 'bold' ? 'checked' : '' }}>
                                <div
                                    class="p-4 rounded-xl border-2 border-surface-700 bg-surface-800 peer-checked:border-primary-500 peer-checked:bg-primary-500/5 transition-all text-center">
                                    <div
                                        class="w-full h-24 bg-white rounded-md mb-3 flex flex-col overflow-hidden border border-surface-600">
                ...
                <div class="space-y-6">
                    <div class="glass rounded-2xl p-6">
                        ...
                        <p class="text-sm text-surface-400 mb-5">These links will appear in the footer of your email
                            template.</p>
                    ...
                    <div class="glass rounded-2xl p-6 flex flex-col items-center">
                        <button type="submit"
                            class="w-full px-6 py-3 rounded-xl bg-gradient-to-r from-primary-500 to-primary-600 text-white text-sm font-bold hover:from-primary-400 hover:to-primary-500 transition-all shadow-lg shadow-primary-500/20">
                            Save Settings
                        </button>
                        @if (session('success'))
                            <div
                                class="mt-4 px-4 py-2 bg-success-500/10 border border-success-500/20 rounded-lg text-success-400 text-sm w-full text-center animate-fade-in">
                                {{ session('success') }}
                            </div>
                        @endif
                    </div>
