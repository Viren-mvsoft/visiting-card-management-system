        ...
        <div class="glass rounded-2xl p-6 animate-fade-in stagger-2">
            <div class="flex items-center justify-between">
                ...
                <div class="w-12 h-12 rounded-xl bg-accent-500/15 flex items-center justify-center">
                    <svg class="w-6 h-6 text-accent-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>
                </div>
            </div>
        </div>
        ...
        <div class="glass rounded-2xl p-6 animate-fade-in stagger-3">
            <div class="flex items-center justify-between mb-3">
                ...
                <div class="w-12 h-12 rounded-xl bg-warning-500/15 flex items-center justify-center">
                    <svg class="w-6 h-6 text-warning-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                </div>
            </div>
            @if($topEvents->count() > 0)
                <div class="space-y-2">
                    @foreach($topEvents->take(3) as $event)
                        <div class="flex items-center justify-between">
                            <span class="text-sm text-surface-300 truncate">{{ $event->event }}</span>
                            <span class="text-xs font-medium px-2 py-0.5 rounded-full bg-surface-800 text-surface-400">{{ $event->count }}</span>
                        </div>
                    @endforeach
                </div>
            @else
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="glass rounded-2xl p-6 animate-fade-in stagger-4">
            <div class="flex items-center justify-between mb-5">
                ...
                <a href="{{ route('contacts.index') }}" class="text-sm text-primary-400 hover:text-primary-300 transition-colors">View all →</a>
            </div>
            @if($recentContacts->count() > 0)
                <div class="space-y-3">
                    @foreach($recentContacts as $contact)
                        <a href="{{ route('contacts.show', $contact) }}" class="flex items-center gap-4 p-3 rounded-xl hover:bg-surface-800/50 transition-all duration-200 group">
                            <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500/20 to-accent-500/20 flex items-center justify-center text-primary-400 font-semibold text-sm border border-primary-500/20">
                                {{ strtoupper(substr($contact->name, 0, 1)) }}
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-surface-200 group-hover:text-white transition-colors truncate">{{ $contact->name }}</p>
                                <p class="text-xs text-surface-500 truncate">{{ $contact->company_name ?? 'No company' }} · {{ $contact->country ?? 'N/A' }}</p>
                            </div>
            ...
            @else
                <div class="text-center py-8">
                    ...
                    <a href="{{ route('contacts.create') }}" class="mt-3 inline-block text-sm text-primary-400 hover:text-primary-300">Add your first contact →</a>
                </div>
            @endif
        </div>
        ...
        <div class="glass rounded-2xl p-6 animate-fade-in stagger-5">
            <div class="flex items-center justify-between mb-5">
                ...
                <a href="{{ route('email-logs.index') }}" class="text-sm text-primary-400 hover:text-primary-300 transition-colors">View all →</a>
            </div>
            @if($recentEmails->count() > 0)
                <div class="space-y-3">
                    @foreach($recentEmails as $log)
                        <div class="flex items-center gap-4 p-3 rounded-xl hover:bg-surface-800/50 transition-all duration-200">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-surface-500 truncate">To: {{ is_array($log->recipients) ? implode(', ', $log->recipients) : $log->recipients }}</p>
                            </div>
