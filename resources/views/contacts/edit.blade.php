...
<div class="max-w-4xl mx-auto animate-fade-in">
    <form method="POST" action="{{ route('contacts.update', $contact) }}" enctype="multipart/form-data">
            ...
            <div id="phone-fields" class="space-y-3">
                @forelse($contact->phones as $i => $phone)
                    <div class="flex items-center gap-3">
                        <select name="phones[{{ $i }}][label]" class="w-32 rounded-xl border border-surface-700 bg-surface-800 px-3 py-2.5 text-sm text-surface-200 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none transition-all">
                        ...
                        <input type="tel" name="phones[{{ $i }}][phone]" value="{{ $phone->phone }}"
                            class="flex-1 rounded-xl border border-surface-700 bg-surface-800 px-4 py-2.5 text-sm text-surface-200 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none transition-all" />
                        <button type="button" onclick="this.parentElement.remove()" class="p-2 rounded-lg text-danger-400 hover:bg-danger-500/10 transition-colors">
                ...
                @empty
                    <div class="flex items-center gap-3">
                        <select name="phones[0][label]" class="w-32 rounded-xl border border-surface-700 bg-surface-800 px-3 py-2.5 text-sm text-surface-200 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none transition-all">
                            <option value="mobile">Mobile</option><option value="office">Office</option><option value="other">Other</option>
                        </select>
        ...
        <div class="glass rounded-2xl p-6 mb-6">
            ...
            <div id="email-fields" class="space-y-3">
                @forelse($contact->emails as $i => $email)
                    <div class="flex items-center gap-3">
                        <select name="emails[{{ $i }}][label]" class="w-32 rounded-xl border border-surface-700 bg-surface-800 px-3 py-2.5 text-sm text-surface-200 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none transition-all">
                    ...
                    <div>
                        ...
                        @if($existingImage)
                            <div class="relative rounded-xl overflow-hidden mb-2 group">
                                <img src="{{ Storage::url($existingImage->file_path) }}" class="w-full h-40 object-cover cursor-pointer" onclick="openLightbox(this.src)" />
                                <button type="button" onclick="confirmDelete('delete-image-{{ $existingImage->id }}', 'image')" class="absolute top-2 right-2 p-1.5 rounded-lg bg-danger-500/80 text-white hover:bg-danger-500 transition-colors opacity-0 group-hover:opacity-100">
                        ...
                        <div class="relative rounded-xl border-2 border-dashed border-surface-700 hover:border-primary-500/50 transition-colors p-4 text-center cursor-pointer" onclick="this.querySelector('input[type=file]').click()">
                            <img id="preview-{{ $name }}" src="" class="hidden w-full h-32 object-cover rounded-lg mb-2" />
                            <div class="upload-placeholder">
                                <svg class="w-6 h-6 mx-auto text-surface-600 mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                                <p class="text-xs text-surface-500">{{ $existingImage ? 'Replace image' : 'Click to upload' }}</p>
                            </div>
        ...
        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('contacts.show', $contact) }}" class="px-5 py-2.5 rounded-xl text-sm font-medium text-surface-400 hover:text-surface-200 transition-colors">Cancel</a>
    ...
    @foreach(['front', 'back', 'other'] as $type)
        @php $existingImage = $contact->images->where('type', $type)->first(); @endphp
        @if($existingImage)
            <form id="delete-image-{{ $existingImage->id }}" method="POST" action="{{ route('contacts.delete-image', [$contact, $existingImage]) }}" class="hidden">
                @csrf @method('DELETE')
            </form>
        @endif
    @endforeach
