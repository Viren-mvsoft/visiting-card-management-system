...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <meta name="description" content="Visiting Card Management System - Digitize, organize, and manage your business contacts">

        <title>{{ config('app.name', 'VCMS') }} — @yield('title', 'Dashboard')</title>
    ...
    <body class="font-sans antialiased bg-surface-950 text-surface-200 min-h-screen">
        ...
        <!-- Flash Messages -->
        @if(session('success'))
            <div data-flash class="fixed top-4 right-4 z-[100] max-w-md px-5 py-3 rounded-xl bg-success-500/15 border border-success-400/30 text-success-400 text-sm font-medium shadow-lg backdrop-blur-md transition-all duration-300 animate-slide-in-right">
        ...
        @endif
        @if(session('error'))
            <div data-flash class="fixed top-4 right-4 z-[100] max-w-md px-5 py-3 rounded-xl bg-danger-500/15 border border-danger-400/30 text-danger-400 text-sm font-medium shadow-lg backdrop-blur-md transition-all duration-300 animate-slide-in-right">
                <div class="flex items-center gap-3">
        ...
        <div class="flex min-h-screen">
            ...
            <div id="sidebar-overlay" class="hidden fixed inset-0 z-40 bg-black/50 backdrop-blur-sm lg:hidden" onclick="toggleSidebar()"></div>

            <!-- Sidebar -->
            <aside id="mobile-sidebar" class="fixed inset-y-0 left-0 z-50 w-72 -translate-x-full lg:translate-x-0 lg:sticky lg:top-0 lg:z-auto lg:h-screen transition-transform duration-300 ease-in-out">
                <div class="flex flex-col h-full bg-surface-900/80 backdrop-blur-xl border-r border-surface-700/50 overflow-y-auto">
                    <!-- Logo -->
                    <div class="flex items-center gap-3 px-6 py-5 border-b border-surface-700/50">
                        ...
                        <div>
                            <h1 class="text-lg font-bold text-white">VCMS</h1>
                    ...
                    <nav class="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
                        ...
                        @if(auth()->user()->isAdmin())
                            <p class="px-3 mt-6 mb-3 text-xs font-semibold uppercase tracking-wider text-surface-500">Admin</p>
                    ...
                    <div class="px-4 py-4 border-t border-surface-700/50">
                        <div data-dropdown class="relative">
                            <button data-dropdown-trigger class="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm hover:bg-surface-800/50 transition-all duration-200">
                                <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center text-white font-semibold text-sm shadow-md">
                            ...
                            <div data-dropdown-content class="absolute bottom-full left-0 right-0 mb-2 rounded-xl bg-surface-800 border border-surface-700 shadow-xl overflow-hidden transition-all duration-200" style="display: none; opacity: 0; transform: scale(0.95);">
                                <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-3 text-sm text-surface-300 hover:bg-surface-700 hover:text-surface-100 transition-colors">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>
                                    Profile
                                </a>
                                <form method="POST" action="{{ route('logout') }}">
                                    @csrf
                                    <button type="submit" class="w-full flex items-center gap-3 px-4 py-3 text-sm text-danger-400 hover:bg-surface-700 transition-colors">
            ...
            <div class="flex-1 flex flex-col min-w-0">
                <!-- Top Bar -->
                <header class="sticky top-0 z-30 flex items-center gap-4 px-6 py-4 bg-surface-950/80 backdrop-blur-xl border-b border-surface-800/50">
                    <!-- Mobile menu button -->
                    <button onclick="toggleSidebar()" class="lg:hidden p-2 rounded-xl text-surface-400 hover:text-surface-200 hover:bg-surface-800 transition-all">
