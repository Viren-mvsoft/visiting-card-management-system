...
<div class="max-w-3xl mx-auto animate-fade-in">
    <form method="POST" action="{{ route('contacts.send-email.store', $contact) }}">
        ...
        <div class="glass rounded-2xl p-6 mb-6">
            ...
            @if($contact->emails->count() > 0)
                <div class="space-y-3">
                    @foreach($contact->emails as $email)
                        <label class="flex items-center gap-3 p-3 rounded-xl bg-surface-800/50 border border-surface-700/50 hover:border-primary-500/30 cursor-pointer transition-all">
            ...
            @else
                <p class="text-sm text-danger-400">No email addresses found for this contact. <a href="{{ route('contacts.edit', $contact) }}" class="underline">Add one</a>.</p>
            @endif
            @error('recipient_emails') <p class="mt-2 text-sm text-danger-400">{{ $message }}</p> @enderror
            ...
            <div class="mt-5 pt-5 border-t border-surface-700/50">
                ...
                <input type="text" name="cc_emails" id="cc_emails" value="{{ old('cc_emails') }}" placeholder="example1@mail.com, example2@mail.com" class="w-full rounded-xl border border-surface-700 bg-surface-800 px-4 py-3 text-sm text-surface-200 focus:border-primary-500 focus:ring-1 focus:ring-primary-500 focus:outline-none transition-all">
                @error('cc_emails') <p class="mt-2 text-sm text-danger-400">{{ $message }}</p> @enderror
            </div>
        </div>
            ...
            @if($configurations->count() > 0)
                <select name="email_configuration_id" id="config-select" required
                    ...
                    @foreach($configurations as $config)
                        <option value="{{ $config->id }}">{{ $config->from_name }} &lt;{{ $config->from_email }}&gt; ({{ $config->name }})</option>
                    @endforeach
                </select>
            @else
                <p class="text-sm text-danger-400">No active email configurations. Ask an admin to configure one.</p>
            @endif
            @error('email_configuration_id') <p class="mt-2 text-sm text-danger-400">{{ $message }}</p> @enderror
        </div>
        ...
        <div class="glass rounded-2xl p-6 mb-6">
            ...
            @if($templates->count() > 0)
                ...
                <div id="template-preview" class="mt-4 p-4 rounded-xl bg-surface-800 border border-surface-700 hidden">
                    ...
                    <div id="preview-body" class="text-sm text-surface-300 prose prose-invert max-w-none"></div>
                </div>
            @else
                <p class="text-sm text-danger-400">No active templates. <a href="{{ route('email-templates.create') }}" class="underline">Create one</a>.</p>
            @endif
            @error('email_template_id') <p class="mt-2 text-sm text-danger-400">{{ $message }}</p> @enderror
        </div>
        ...
        <div class="glass rounded-2xl p-6 mb-6">
            ...
            <div class="space-y-3">
                @foreach(['front' => 'Card Front Image', 'back' => 'Card Back Image', 'other' => 'Other Image'] as $type => $label)
                    ...
                    <label class="flex items-center gap-4 p-3 rounded-xl bg-surface-800/50 border border-surface-700/50 {{ $img ? 'hover:border-primary-500/30 cursor-pointer' : 'opacity-50 cursor-not-allowed' }} transition-all">
                        <input type="checkbox" name="attach_{{ $type }}" value="1" {{ $img ? 'checked' : 'disabled' }}
                            class="rounded border-surface-600 bg-surface-800 text-primary-500 focus:ring-primary-500 focus:ring-offset-0" />
                        @if($img)
                            <img src="{{ Storage::url($img->file_path) }}" class="w-12 h-12 rounded-lg object-cover" />
                        @else
                            <div class="w-12 h-12 rounded-lg bg-surface-800 border border-surface-700 flex items-center justify-center">
                                <svg class="w-5 h-5 text-surface-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14"/></svg>
                            </div>
                        @endif
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('contacts.show', $contact) }}" class="px-5 py-2.5 rounded-xl text-sm font-medium text-surface-400 hover:text-surface-200 transition-colors">← Back to Contact</a>
            <button type="submit" class="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-accent-500 to-accent-600 text-white text-sm font-semibold hover:from-accent-400 hover:to-accent-500 transition-all shadow-lg shadow-accent-500/20 hover:shadow-xl">
